...
public class EstacionamentoServiceImp implements EstacionamentoService{
    ...
    @Override
    public List<EstacionamentoCompletoDto> obterPedidos() {
        List<Estacionamento> pedidos = repository.findAll();
        List<EstacionamentoCompletoDto> dto = pedidos.stream().map(p -> new ModelMapper().map(p, EstacionamentoCompletoDto.class)).collect(Collectors.toList());
        return dto;
    }
